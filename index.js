...
const {
  ...
  ButtonBuilder,
  ButtonStyle,
  AuditLogEvent // Kimin sildiğini bulmak için bu eklendi
} = require("discord.js");
...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.GuildModeration // Denetim kayıtlarına erişim için bu intent şarttır
  ],
  partials: [
    Partials.Message,
    Partials.Channel,
...
const BAD_WORDS = [
  // Türkçe
  "amk", "a.mk", "aq", "amq", "amına", "amina", "amini", "amcık", "amcik", "orospu", "orosb", "oc", "oç", "piç", "pic", "sik", "sikerim", "siktir", "sikik", "yarak", "yarrak", "göt", "got", "götveren", "ibne", "eşşek", "mal", "salak", "gerizekalı", "aptal", "kahpe", "pezevenk", "şerefsiz", "haysiyetsiz", "ananı", "anan", "bacını", "bacin", "ananıskm", "anneskm", "mk", "aq", "sg", "sg git", "yavşak", "lavuk", "dangalak", "geri zekalı", "gerizekali", "aptal herif", "sürtük", "puşt", "it oğlu it", "salak oç", "am biti", "gavat", "dalyarak", "am hoşafı", "amcık hoşafı", "dingil", "gerzek", "malsın", "ezik", "am surat", "sik kafalı", "göt kafa", "beyinsiz",
  // İngilizce
  "fuck", "fucking", "motherfucker", "bitch", "shit", "asshole", "dick", "wtf", "idiot", "stupid", "bastard", "slut", "whore", "retard", "dumbass", "moron", "loser", "nigga", "nigger", "faggot", "sucker", "jerk", "piece of shit"
];
...
client.on("messageDelete", async (message) => {
  ...
  // Mesajı silen kişiyi bulmak için denetim kaydını sorguluyoruz
  let sileyenKim = "Kendi sildi veya Tespit edilemedi";
  
  try {
    // Botun denetim kaydını görme yetkisi olup olmadığını kontrol ediyoruz
    if (message.guild.members.me.permissions.has(PermissionsBitField.Flags.ViewAuditLog)) {
      // Mesaj silme işlemlerine ait son kaydı çekiyoruz
      ...
      if (deletionLog) {
        ...
        // Eğer silinen mesajın sahibi ile denetim kaydındaki hedef kişi uyuşuyorsa
        // ve bu işlem son 5 saniye içinde gerçekleştiyse mesajı silen kişiyi alıyoruz
        if (target.id === message.author?.id && (Date.now() - deletionLog.createdTimestamp) < 5000) {
          sileyenKim = `<@${executor.id}> (${executor.tag})`;
        }
      }
    }
  } catch (error) {
    console.error("Denetim kaydı çekilirken hata oluştu:", error);
  }

  log.send(
    `🗑️ **MESAJ SİLİNDİ**\n\n` +
    `👤 **Mesaj Sahibi:** ${message.author?.tag || "Bilinmiyor"} (<@${message.author?.id || "0"}>)\n` +
    `🛠️ **Silen Kişi:** ${sileyenKim}\n` +
    `💬 **Mesaj:** ${message.content || "*Mesaj içeriği bulunamadı veya bot görmeden silindi*"}`
  );
});
...
client.on("messageCreate", async (message) => {
  ...
  if (BAD_WORDS.some(word => clean.includes(word))) {
    ...
    if (curse[id] >= 3) {
      ...
      return message.channel.send("⚠️ 3 küfür → 5 dakika mute");
    }
    return message.channel.send(`⚠️ Küfür uyarısı (${curse[id]}/3)`);
  }
  ...
  if (message.content === "!xp") return message.reply(`⭐ XP'n: ${xp[id]}`);
  if (message.content === "!param") return message.reply(`💰 Paran: ${money[id]}`);
  ...
  if (message.content === "!top10") {
    const top = Object.entries(xp)
      ...
      .join("\n");
    return message.channel.send(`🏆 TOP 10 XP\n\n${top}`);
  }
  ...
  if (message.content === "!shop") {
    ...
    return message.channel.send({ content: "🛒 SHOP MENÜSÜ", components: [row] });
  }
  ...
  if (message.content.startsWith("!cekilis")) {
    ...
    setTimeout(() => {
      ...
      if (!users || users.length <= 0) return message.channel.send("❌ Çekilişe kimse katılmadı");

      const winner = users[Math.floor(Math.random() * users.length)];
      message.channel.send(`🏆 Kazanan: <@${winner}>`);

      const log = message.guild.channels.cache.get(LOG_CHANNEL_ID);
      if (log) log.send(`🎉 Çekiliş bitti → Kazanan: <@${winner}>`);
    }, ms);
  }
  ...
  if (message.content.startsWith("!xpver")) {
    ...
    if (!member) return message.reply("Kullanıcı belirt");
    if (!amount) return message.reply("Miktar belirt");
    ...
    return message.reply(`⭐ ${amount} XP verildi`);
  }
});
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.customId === "join_giveaway") {
    if (!giveaways[interaction.message.id]) giveaways[interaction.message.id] = [];
    if (giveaways[interaction.message.id].includes(id)) {
  ...
  if (interaction.customId === "buy_senor") {
    if (money[id] < 250000) {
      return interaction.reply({ content: "❌ 250.000 para lazım", ephemeral: true });
    }
    money[id] -= 250000;
    save("./data/money.json", money);
    ...
    return interaction.reply({ content: "👑 SENOR rolünü aldın!", ephemeral: true });
  }
});
